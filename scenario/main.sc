theme: /
    ...
    state: Welcome
        ...
        event!: RUN_APP
        a: Приветствую! Это голосовой битмейкер для создания музыки. Какую команду выполнить?
        ...
        script:
            ...
            if (num !== -1) {
                if (num >= 1 && num <= 16) {
                    ...
                    $reactions.answer("Добавлено! Какую команду выполнить дальше?");
                } else {
                    $reactions.answer("Сетка состоит только из шестнадцати тактов. Требуется выбрать число от 1 до 16.");
                }
            } else {
                $reactions.answer("Не удалось распознать номер такта. Требуется назвать число от 1 до 16.");
            }
    ...
    state: Reset
        ...
        script:
            ...
            $reactions.answer("Сетка очищена! Поле готово к новой композиции. Какую команду выполнить?");

    state: CatchAll
        event!: noMatch
        a: Команда не распознана. Можно сказать «добавь бочку на пятый такт», «очисти всё» или попросить «помощь». Какую команду выполнить?

    state: Help
        q!: * (помощь|справка|что ты умеешь|как играть|инструкция|что делать) *
        a: Это интерактивный сиквенсор для создания музыкальных лупов. Требуется назвать инструмент и номер такта от 1 до 16. Например: "добавь бочку на пятый такт". Для полного сброса поля подходит команда "очисти всё". Какую команду выполнить?